    ...
    async analyzeProject(projectPath: string): Promise<ProjectAnalysis> {
        ...
        try {
            ...
            try {
                ...
                if (pkg.name) keywords.add(pkg.name.replace(/-/g, ' '));
                if (pkg.description) keywords.add(pkg.description);
                if (pkg.keywords) pkg.keywords.forEach((k: string) => keywords.add(k));
